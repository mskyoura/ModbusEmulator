...
namespace ModbusEmulator.Tests
{
    ...
    public class CommandParseTests
    {
        ...
        public void Parse_WriteCommandWithRelayData_ShouldReturnValidCommand()
        {
            ...
            Assert.That(result.RelaysData, Has.Count.EqualTo(3), "Должно быть 3 элемента данных реле");
            ...
            Assert.That(relay0.RelayStatus, Is.EqualTo(RelayStatus.OFF), "Статус первого реле должен быть OFF (255)");
            Assert.That(relay0.Delay, Is.EqualTo(6553.5), "Задержка первого реле должна быть 6553.5 (0xFFFF * 0.1)");
            Assert.That(relay0.Duration, Is.EqualTo(255), "Длительность первого реле должна быть 255");
            ...
            // Проверяем третье реле
            Assert.That(result.RelaysData.ContainsKey(2), Is.True, "Должны быть данные для реле 2");
            var relay2 = result.RelaysData[2];
            Assert.That(relay2.RelayStatus, Is.EqualTo(RelayStatus.OFF), "Статус третьего реле должен быть OFF (255)");
            Assert.That(relay2.Delay, Is.EqualTo(6553.5), "Задержка третьего реле должна быть 6553.5 (0xFFFF * 0.1)");
            Assert.That(relay2.Duration, Is.EqualTo(255), "Длительность третьего реле должна быть 255");
        }
        ...
        public void Parse_GroupWriteCommandWithRelayData_ShouldReturnValidCommand()
        {
            ...
            Assert.That(result.RelaysData.ContainsKey(1), Is.True, "Должны быть данные для реле 1");
            var relay1 = result.RelaysData[1];
            Assert.That(relay1.RelayStatus, Is.EqualTo(RelayStatus.OFF), "Статус второго реле должен быть OFF (255)");
            Assert.That(relay1.Delay, Is.EqualTo(6553.5), "Задержка второго реле должна быть 6553.5 (0xFFFF * 0.1)");
            Assert.That(relay1.Duration, Is.EqualTo(255), "Длительность второго реле должна быть 255");
